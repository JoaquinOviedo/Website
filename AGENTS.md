...
- `docs/ARCHITECTURE.md`: dependency direction, folder ownership, hotspots, and safe change sequence.
- `docs/ADDING_PROJECTS.md`: complete checklist for project data, evidence, routes, and prototypes.
- `docs/PRODUCT_DECISIONS.md`: recruiter-first UX decisions that future iterations must preserve.
- `docs/CONTENT_GUIDE.md`: factual editing workflow and pending confirmations.
...
- `content/profile.ts`: public profile and asset configuration.
- `content/projects/`: one typed record per project; register display order in `content/projects/index.ts`.
- `content/projectMedia.ts`: sanitized gallery evidence and localized descriptions.
- `content/copy.ts`: shared visible Spanish and English copy.
- `content/pending.ts`: private reminders only; never render it.
...
- `public/cv/`: replaceable CV files; keep paths synchronized with `content/profile.ts`.
...
## Change rules
1. Update facts in their focused `content/` file; update translated interface copy in `content/copy.ts`.
...
7. Keep landing-page project cards compact. Galleries, prototypes, and detailed process evidence belong in case-study routes.
8. Do not grow `content/portfolio.ts`; it is a compatibility barrel. New projects get their own file under `content/projects/`.
