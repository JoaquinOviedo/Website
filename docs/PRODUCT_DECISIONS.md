...
- The about section uses three concise, human paragraphs covering current work, problem ownership, and responsible AI-assisted practice. Preserve the paragraph structure instead of collapsing it into a dense block or an abstract principle list.
- On wide screens, About uses an editorial two-column layout: the professional introduction leads on the left and working style plus AI practice sit on the right. Do not fill the open space with redundant skills or decorative slogans; collapse to one column on small screens.
...
- Text inputs, textareas, and selects indicate keyboard focus by strengthening their own border with an inset accent, never by drawing a second ring outside the control. Links and buttons retain an external focus outline where it improves keyboard orientation.
